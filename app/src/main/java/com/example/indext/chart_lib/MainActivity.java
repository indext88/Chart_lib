package com.example.indext.chart_lib;
...
import com.example.indext.chart_lib.R;
...
public class MainActivity extends AppCompatActivity {
    ...
    //线形图
    private double[] temperature = {3.36,3.16,3.45};//图表的数据点
    //X轴的标注
    private String[] lineData = {"大一上期","大一下期","大二上期"};
    ...
    private int[] pieData = {7, 18, 21, 25,29};//饼状图中的数据
    ...
    private String[] stateChar = {"1.0-2.0", "2.0-3.0", "3.0-3.5", "3.5-4.0","4.0"};
    //柱状图
    private String[] year = new String[]{"3.5以上(大一上)", "3.5以上(大一下)", "3.5以上(大二上)"};
    private ColumnChartView columnChartView;
    private ColumnChartData columnData;
    private int[] columnY = {1,2,3,4,5,6};
    ...
    private void setAxisPoints() {
        for (int i = 0; i < temperature.length; i++) {
            pointValues.add(new PointValue(i, (float) temperature[i]));
    ...
    private void initLineChart() {
        ...
        //X轴
        Axis axisX = new Axis();
        axisX.setHasTiltedLabels(false);    //X轴字体是斜的显示还是直的，true是倾斜显示
    ...
    private void initPieChart() {
        ...
        pieCharData.setCenterText1("科目学绩数");//设置中心圆默认显示的文字
        pieCharData.setCenterText1FontSize(30);
    ...
    private void initColumnChart() {
        ...
        for (int k = 0; k < columnY.length; k++) {
            axisYValues.add(new AxisValue(k).setValue((float) columnY[k]));
        }
        for (int i = 0; i < year.length; ++i) {
            ...
            for (int j = 0; j < 1; ++j) {
                switch (i + 1) {
                    case 1:
                        subcolumnValues.add(new SubcolumnValue(6, ChartUtils.COLOR_BLUE));
                        break;
                    case 2:
                        subcolumnValues.add(new SubcolumnValue(4, ChartUtils.COLOR_GREEN));
                        break;
                    case 3:
                        subcolumnValues.add(new SubcolumnValue(5, ChartUtils.COLOR_RED));
                        break;
                }
            }
            // 点击柱状图就展示数据量
